...
contract BorrowerOperations is LiquityBase, Ownable, CheckContract, IBorrowerOperations {
    ...
    // A doubly linked list of Troves, sorted by their ore_token ratios
    ...
    // --- Events ---
    // All events are inherited from IBorrowerOperations
    ...
    function openTrove(uint _maxFeePercentage, uint _LUSDAmount, address _upperHint, address _lowerHint) external payable override {
        ...
        if (!isRecoveryMode) {
            ...
            vars.netDebt = vars.netDebt + vars.LUSDFee; // Using '+' instead of add
        }
        ...
        // Move the ore_tokener to the Active Pool, and mint the LUSDAmount to the borrower
        ...
        emit TroveUpdated(msg.sender, vars.compositeDebt, msg.value, vars.stake, uint8(BorrowerOperation.openTrove)); // Cast enum to uint8
        emit LUSDBorrowingFeePaid(msg.sender, vars.LUSDFee);
    }

    // Send NEON_TOKEN as ore_token to a trove
    function addColl(address _upperHint, address _lowerHint) external payable override {
        _adjustTrove(msg.sender, 0, 0, false, _upperHint, _lowerHint, 0);
    }

    // Send NEON_TOKEN as ore_token to a trove. Called by only the Stability Pool.
    function moveNEON_TOKENGainToTrove(address _borrower, address _upperHint, address _lowerHint) external payable override {
        _requireCallerIsStabilityPool();
        _adjustTrove(_borrower, 0, 0, false, _upperHint, _lowerHint, 0);
    }

    // Withdraw NEON_TOKEN ore_token from a trove
    ...
    /*
    * _adjustTrove(): Alongside a debt change, this function can perform either a ore_token top-up or a ore_token withdrawal. 
    *
    ...
    */
    function _adjustTrove(address _borrower, uint _collWithdrawal, uint _LUSDChange, bool _isDebtIncrease, address _upperHint, address _lowerHint, uint _maxFeePercentage) internal {
        ...
        // Confirm the operation is either a borrower adjusting their own trove, or a pure NEON_TOKEN transfer from the Stability Pool to a trove
        assert(msg.sender == _borrower || (msg.sender == stabilityPoolAddress && msg.value > 0 && _LUSDChange == 0));

        contractsCache.troveManager.applyPendingRewards(_borrower);

        // Get the collChange based on whore_tokener or not NEON_TOKEN was sent in the transaction
        ...
        if (_isDebtIncrease && !isRecoveryMode) { 
            ...
            vars.netDebtChange = vars.netDebtChange + vars.LUSDFee; // The raw debt change includes the fee // Using '+' instead of add
        }
        ...
        if (!_isDebtIncrease && _LUSDChange > 0) {
            _requireAtLeastMinNetDebt(_getNetDebt(vars.debt) - vars.netDebtChange); // Using '-' instead of sub
        ...
        emit TroveUpdated(_borrower, vars.newDebt, vars.newColl, vars.stake, uint8(BorrowerOperation.adjustTrove)); // Cast enum to uint8
        emit LUSDBorrowingFeePaid(msg.sender,  vars.LUSDFee);

        // Use the unmodified _LUSDChange here, as we don't send the fee to the user
        _moveTokensAndNEON_TOKENfromAdjustment(
    ...
    function closeTrove() external override {
        ...
        _requireSufficientLUSDBalance(lusdTokenCached, msg.sender, debt - LUSD_GAS_COMPENSATION); // Using '-' instead of sub
        ...
        emit TroveUpdated(msg.sender, 0, 0, 0, uint8(BorrowerOperation.closeTrove)); // Cast enum to uint8

        // Burn the repaid LUSD from the user's balance and the gas compensation from the Gas Pool
        _repayLUSD(activePoolCached, lusdTokenCached, msg.sender, debt - LUSD_GAS_COMPENSATION); // Using '-' instead of sub
        _repayLUSD(activePoolCached, lusdTokenCached, gasPoolAddress, LUSD_GAS_COMPENSATION);

        // Send the ore_token back to the user
        activePoolCached.sendNEON_TOKEN(msg.sender, coll);
    }

    /**
     * Claim remaining ore_token from a redemption or from a liquidation with ICR > MCR in Recovery Mode
     */
    function claimORE_TOKEN() external override {
        // send NEON_TOKEN from CollSurplus Pool to owner
    ...
    function _getUSDValue(uint _coll, uint _price) internal pure returns (uint) {
        uint usdValue = (_price * _coll) / DECIMAL_PRECISION; // Using '*' and '/' instead of mul/div
    ...
    // Update trove's coll and debt based on whore_tokener they increase or decrease
    ...
    function _moveTokensAndNEON_TOKENfromAdjustment
    (
        ...
        internal
    {
        ...
        if (_isCollIncrease) {
            ...
            _activePool.sendNEON_TOKEN(_borrower, _collChange);
        }
    }

    // Send NEON_TOKEN to Active Pool and increase its recorded NEON_TOKEN balance
    function _activePoolAddColl(IActivePool _activePool, uint _amount) internal {
        (bool success, ) = address(_activePool).call{value: _amount}("");
        require(success, "BorrowerOps: Sending NEON_TOKEN to ActivePool failed");
    }
    ...
    function _requireNonZeroAdjustment(uint _collWithdrawal, uint _LUSDChange) internal view {
        require(msg.value != 0 || _collWithdrawal != 0 || _LUSDChange != 0, "BorrowerOps: There must be either a ore_token change or a debt change");
    }
    ...
    function _requireNoCollWithdrawal(uint _collWithdrawal) internal pure {
        require(_collWithdrawal == 0, "BorrowerOps: ORE_TOKEN withdrawal not permitted Recovery Mode");
    }
        ...
        view 
    {
        ...
        *
        * - Pure ore_token top-up
        * - Pure debt repayment
        * - ORE_TOKEN top-up with debt repayment
        * - A debt increase combined with a ore_token top-up which makes the ICR >= 150% and improves the ICR (and by extension improves the TCR).
        *
    ...
    // Compute the new ore_token ratio, considering the change in coll and debt. Assumes 0 pending rewards.
        ...
        returns (uint, uint)
    {
        ...
        newColl = _isCollIncrease ? _coll + _collChange :  _coll - _collChange; // Using '+' and '-' instead of add/sub
        newDebt = _isDebtIncrease ? _debt + _debtChange : _debt - _debtChange; // Using '+' and '-' instead of add/sub
        ...
        returns (uint)
    {
        ...
        totalColl = _isCollIncrease ? totalColl + _collChange : totalColl - _collChange; // Using '+' and '-' instead of add/sub
        totalDebt = _isDebtIncrease ? totalDebt + _debtChange : totalDebt - _debtChange; // Using '+' and '-' instead of add/sub
