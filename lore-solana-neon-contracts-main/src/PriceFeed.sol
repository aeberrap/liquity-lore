...
/*
* PriceFeed for mainnet deployment, to be connected to Chainlink's live NEON_TOKEN:USD aggregator reference 
...
contract PriceFeed is Ownable, CheckContract, BaseMath, IPriceFeed {
    ...
    uint constant public NEON_TOKENUSD_TELLOR_REQ_ID = 1;
    ...
    */
    ...
    // LastGoodPriceUpdated is inherited from IPriceFeed
    ...
    */
    ...
    function _chainlinkIsFrozen(ChainlinkResponse memory _response) internal view returns (bool) {
        return block.timestamp - _response.timestamp > TIMEOUT; // Using '-'
    }
    ...
    function _chainlinkPriceChangeAboveMax(ChainlinkResponse memory _currentResponse, ChainlinkResponse memory _prevResponse) internal pure returns (bool) {
        ...
        */
        uint percentDeviation = (maxPrice - minPrice) * DECIMAL_PRECISION / maxPrice; // Using '-', '*', '/'
    ...
    function _tellorIsBroken(TellorResponse memory _response) internal view returns (bool) {
     ...
     function _tellorIsFrozen(TellorResponse  memory _tellorResponse) internal view returns (bool) {
        return block.timestamp - _tellorResponse.timestamp > TIMEOUT; // Using '-'
    }
    ...
    function _bothOraclesSimilarPrice( ChainlinkResponse memory _chainlinkResponse, TellorResponse memory _tellorResponse) internal pure returns (bool) {
        ...
        uint percentPriceDifference = (maxPrice - minPrice) * DECIMAL_PRECISION / minPrice; // Using '-', '*', '/'
    ...
    function _scaleChainlinkPriceByDigits(uint _price, uint _answerDigits) internal pure returns (uint) {
        ...
        */
        ...
        if (_answerDigits >= TARGET_DIGITS) {
            ...
            price = _price / (10 ** (_answerDigits - TARGET_DIGITS)); // Using '/'
        }
        else if (_answerDigits < TARGET_DIGITS) {
            // Scale the returned price value up to Liquity's target precision
            price = _price * (10 ** (TARGET_DIGITS - _answerDigits)); // Using '*'
        }
        return price;
    }

    function _scaleTellorPriceByDigits(uint _price) internal pure returns (uint) {
        return _price * (10**(TARGET_DIGITS - TELLOR_DIGITS)); // Using '*'
    }
    ...
    function _getCurrentTellorResponse() internal view returns (TellorResponse memory tellorResponse) {
        try tellorCaller.getTellorCurrentValue(NEON_TOKENUSD_TELLOR_REQ_ID) returns
        (
            bool ifRetrieve,
            uint256 value,
