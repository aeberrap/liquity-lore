...
contract StabilityPool is LiquityBase, Ownable, CheckContract, IStabilityPool {
    ...
    */
    ...
    /* NEON_TOKEN Gain sum 'S': During its lifetime, each deposit d_t earns an NEON_TOKEN gain of ( d_t * [S - S_t] )/P_t, where S_t
    ...
    */
    ...
    uint public lastNEON_TOKENError_Offset;
    ...
    * - Sends depositor's accumulated gains (LQTY, NEON_TOKEN) to depositor
    ...
    */
    function provideToSP(uint _amount, address _frontEndTag) external override {
        ...
        uint depositorNEON_TOKENGain = getDepositorNEON_TOKENGain(msg.sender);
        ...
        emit NEON_TOKENGainWithdrawn(msg.sender, depositorNEON_TOKENGain, LUSDLoss); // LUSD Loss required for event log

        _sendNEON_TOKENGainToDepositor(depositorNEON_TOKENGain);
     }
    ...
    * - Sends all depositor's accumulated gains (LQTY, NEON_TOKEN) to depositor
    * - Sends the tagged front end's accumulated LQTY gains to the tagged front end
    ...
    */
    function withdrawFromSP(uint _amount) external override {
        if (_amount !=0) {_requireNoUnderORE_TOKENizedTroves();}
        uint initialDeposit = deposits[msg.sender].initialValue;
    ...
    /* withdrawNEON_TOKENGainToTrove:
    * - Triggers a LQTY issuance, based on time passed since the last issuance. The LQTY issuance is shared between *all* depositors and front ends
    * - Sends all depositor's LQTY gain to  depositor
    * - Sends all tagged front end's LQTY gain to the tagged front end
    * - Transfers the depositor's entire NEON_TOKEN gain from the Stability Pool to the caller's trove
    * - Leaves their compounded deposit in the Stability Pool
    * - Updates snapshots for deposit and tagged front end stake */
    function withdrawNEON_TOKENGainToTrove(address _upperHint, address _lowerHint) external override {
        uint initialDeposit = deposits[msg.sender].initialValue;
        _requireUserHasDeposit(initialDeposit);
        _requireUserHasTrove(msg.sender);
        _requireUserHasNEON_TOKENGain(msg.sender);
        ...
        /* Emit events before transferring NEON_TOKEN gain to Trove.
         This lets the event log make more sense (i.e. so it appears that first the NEON_TOKEN gain is withdrawn
        and then it is deposited into the Trove, not the other way around). */
        emit NEON_TOKENGainWithdrawn(msg.sender, depositorNEON_TOKENGain, LUSDLoss);
        emit UserDepositChanged(msg.sender, compoundedLUSDDeposit);

        NEON_TOKEN = NEON_TOKEN - depositorNEON_TOKENGain; // Using '-'
        emit StabilityPoolNEON_TOKENBalanceUpdated(NEON_TOKEN);
        emit EtherSent(msg.sender, depositorNEON_TOKENGain);

        borrowerOperations.moveNEON_TOKENGainToTrove{ value: depositorNEON_TOKENGain }(msg.sender, _upperHint, _lowerHint);
    }
    ...
    * and transfers the Trove's NEON_TOKEN ore_token from ActivePool to StabilityPool.
    ...
    */
    function offset(uint _debtToOffset, uint _collToAdd) external override {
        ...
        (uint NEON_TOKENGainPerUnitStaked,
            uint LUSDLossPerUnitStaked) = _computeRewardsPerUnitStaked(_collToAdd, _debtToOffset, totalLUSD);

        _updateRewardSumAndProduct(NEON_TOKENGainPerUnitStaked, LUSDLossPerUnitStaked);  // updates S and P
    ...
    function _computeRewardsPerUnitStaked(
        ...
        internal
        returns (uint NEON_TOKENGainPerUnitStaked, uint LUSDLossPerUnitStaked)
    {
        /*
        * Compute the LUSD and NEON_TOKEN rewards. Uses a "feedback" error correction, to keep
        ...
        * 5) Note: static analysis tools complain about this "division before multiplication", however, it is intended.
        */
        uint NEON_TOKENNumerator = (_collToAdd * DECIMAL_PRECISION) + lastNEON_TOKENError_Offset; // Using '*' and '+'
         ...
         * As we are doing floor + 1 in the division, it will still offset somore_tokening
         */
        ...
        NEON_TOKENGainPerUnitStaked = NEON_TOKENNumerator / _totalLUSDDeposits; // Using '/'
        lastNEON_TOKENError_Offset = NEON_TOKENNumerator - (NEON_TOKENGainPerUnitStaked * _totalLUSDDeposits); // Using '-' and '*'

        return (NEON_TOKENGainPerUnitStaked, LUSDLossPerUnitStaked);
    }

    // Update the Stability Pool reward sum S and product P
    function _updateRewardSumAndProduct(uint _NEON_TOKENGainPerUnitStaked, uint _LUSDLossPerUnitStaked) internal {
        uint currentP = P;
        uint newP;
        ...
        * The NEON_TOKEN gain for any given depositor from a liquidation depends on the value of their deposit
        * (and the value of totalDeposits) prior to the Stability being depleted by the debt in the liquidation.
        *
        * Since S corresponds to NEON_TOKEN gain, and P to deposit loss, we update S first.
        */
        uint marginalNEON_TOKENGain = _NEON_TOKENGainPerUnitStaked * currentP; // Using '*'
        uint newS = currentS + marginalNEON_TOKENGain; // Using '+'
    ...
    function _moveOffsetCollAndDebt(uint _collToAdd, uint _debtToOffset) internal {
        ...
        activePoolCached.sendNEON_TOKEN(address(this), _collToAdd);
    }
    ...
    /* Calculates the NEON_TOKEN gain earned by the deposit since its last snapshots were taken.
    * Given by the formula:  E = d0 * (S - S(0))/P(0)
    * where S(0) and P(0) are the depositor's snapshots of the sum S and product P, respectively.
    * d0 is the last recorded deposit value.
    */
    function getDepositorNEON_TOKENGain(address _depositor) public view override returns (uint) {
        ...
        uint NEON_TOKENGain = _getNEON_TOKENGainFromSnapshots(initialDeposit, snapshots);
        return NEON_TOKENGain;
    }

    function _getNEON_TOKENGainFromSnapshots(uint initialDeposit, Snapshots memory snapshots) internal view returns (uint) {
        /*
        * Grab the sum 'S' from the scale at which the stake was made. The NEON_TOKEN gain may span up to one scale change.
        * If it does, the second portion of the NEON_TOKEN gain is scaled by 1e9.
        ...
        */
        ...
        uint NEON_TOKENGain = (initialDeposit * (firstPortion + secondPortion)) / P_Snapshot / DECIMAL_PRECISION; // Using '*', '+', '/'
    ...
    */
        ...
        returns (uint)
    {
        ...
        *
        * Thus it's unclear whore_tokener this line is still really needed.
        */
    ...
    // --- Sender functions for LUSD deposit, NEON_TOKEN gains and LQTY gains ---
    ...
    function _sendNEON_TOKENGainToDepositor(uint _amount) internal {
        if (_amount == 0) {return;}
        uint newNEON_TOKEN = NEON_TOKEN - _amount; // Using '-'
        NEON_TOKEN = newNEON_TOKEN;
        emit StabilityPoolNEON_TOKENBalanceUpdated(newNEON_TOKEN);
        emit EtherSent(msg.sender, _amount);

        (bool success, ) = msg.sender.call{ value: _amount }("");
        require(success, "StabilityPool: sending NEON_TOKEN failed");
    }
    ...
    function _requireNoUnderORE_TOKENizedTroves() internal {
    ...
    function _requireUserHasTrove(address _depositor) internal view {
        require(troveManager.getTroveStatus(_depositor) == 1, "StabilityPool: caller must have an active trove to withdraw NEON_TOKENGain to");
    }

    function _requireUserHasNEON_TOKENGain(address _depositor) internal view {
        uint NEON_TOKENGain = getDepositorNEON_TOKENGain(_depositor);
        require(NEON_TOKENGain > 0, "StabilityPool: caller must have non-zero NEON_TOKEN Gain");
    }
    ...
    receive() external payable {
        ...
        NEON_TOKEN = NEON_TOKEN + msg.value; // Using '+'
        emit StabilityPoolNEON_TOKENBalanceUpdated(NEON_TOKEN);
    }
}
