...
contract HintHelpers is LiquityBase, Ownable, CheckContract {
    ...
    /* getRedemptionHints() - Helper function for finding the right hints to pass to redeemORE_TOKEN().
     *
        ...
        )
    {
        ...
        if (_maxIterations == 0) {
            _maxIterations = type(uint256).max; // Replace uint(-1)
        }
        ...
        while (currentTroveuser != address(0) && remainingLUSD > 0 && _maxIterations-- > 0) {
            uint netLUSDDebt = _getNetDebt(troveManager.getTroveDebt(currentTroveuser)) +
                troveManager.getPendingLUSDDebtReward(currentTroveuser); // Replace .add

            if (netLUSDDebt > remainingLUSD) {
                if (netLUSDDebt > MIN_NET_DEBT) {
                    uint maxRedeemableLUSD = LiquityMath._min(remainingLUSD, netLUSDDebt - MIN_NET_DEBT); // Replace .sub

                    uint NEON_TOKEN = troveManager.getTroveColl(currentTroveuser) +
                        troveManager.getPendingNEON_TOKENReward(currentTroveuser); // Replace .add

                    uint newColl = NEON_TOKEN - (maxRedeemableLUSD * DECIMAL_PRECISION / _price); // Replace .sub, .mul, .div
                    uint newDebt = netLUSDDebt - maxRedeemableLUSD; // Replace .sub

                    uint compositeDebt = _getCompositeDebt(newDebt);
                    partialRedemptionHintNICR = LiquityMath._computeNominalCR(newColl, compositeDebt);

                    remainingLUSD = remainingLUSD - maxRedeemableLUSD; // Replace .sub
                }
                break;
            } else {
                remainingLUSD = remainingLUSD - netLUSDDebt; // Replace .sub
            }

            currentTroveuser = sortedTrovesCached.getPrev(currentTroveuser);
        }

        truncatedLUSDamount = _LUSDamount - remainingLUSD; // Replace .sub
    }
